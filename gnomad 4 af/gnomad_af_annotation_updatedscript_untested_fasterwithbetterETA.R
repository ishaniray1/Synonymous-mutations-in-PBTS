...
annotate_variants_batch <- function(variants_df, 
  ...
  for (i in start_idx:total) {
    ...
    tryCatch({
      ...
      # Reduced to 0.02 for faster runs (still respectful)
      Sys.sleep(0.02)
    ...
    if (i %% 100 == 0) {
      # Calculate more realistic ETA based on actual elapsed time
      if (i == 100) {
        start_time <<- Sys.time()
      }
      ...
      if (exists("start_time")) {
        ...
        rate <- (i - 100) / elapsed  # variants per minute
        remaining <- total - i
        eta_mins <- if (rate > 0) remaining / rate else (total - i) * 0.05 / 60
      } else {
        eta_mins <- (total - i) * 0.05 / 60
      }
      
      pct <- round(100 * i / total, 1)
      cat(sprintf("Progress: %d/%d (%s%%) - ETA: ~%.0f min\n", 
                  i, total, pct, eta_mins))
