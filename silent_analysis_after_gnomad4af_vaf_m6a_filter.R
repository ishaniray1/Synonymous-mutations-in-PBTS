...
library(readr)
silent_variants_df <- read_csv("annotated_1_complete_1107.csv")
...
num_cols <- c("t_alt_count","t_depth","n_alt_count","n_depth","gnomad_3_1_1_AF", "gnomAD_4_AF")
mut <- mut %>% mutate(across(all_of(num_cols), ~ as.numeric(.)))
mut$gnomAD_4_AF=as.numeric(mut$gnomAD_4_AF)
mut$gnomAD_4_AF[is.na(as.numeric(mut$gnomAD_4_AF))]=0
#mut=mut[as.numeric(mut$gnomAD_4_AF)<0.005, ] #not using gnmoad filtering for now
...
#mut <- mut[mut$fracN <= 0, ] #exclude mutations with any reads in ref/normal sample
...
combined_m6a_unique <- readRDS("Combined_Exon_m6A_all_datasets_unique.rds")
head(combined_m6a_unique)
str(combined_m6a_unique)
chroms_atlas <- sub("^chr", "", combined_m6a_unique$chromosome, ignore.case=TRUE)
atlas_gr <- GRanges(
  seqnames = chroms_atlas,
  ranges = IRanges(start = combined_m6a_unique$start, end = combined_m6a_unique$end),
  strand = combined_m6a_unique$strand,
  gene_name = combined_m6a_unique$gene_name,
  source = combined_m6a_unique$source
)

merip_final_data <- readRDS("merip_exons_hg38.rds")
head(merip_final_data)
str(merip_final_data)
chroms_atlas_merip <- sub("^chr", "", merip_final_data$chromosome, ignore.case=TRUE)
table(merip_final_data$strand, useNA = "ifany")
merip_final_data$strand <- ifelse(
  merip_final_data$strand %in% c("+", "-"),
  merip_final_data$strand,
  "*"   # replace NA, ., ?, blank) with "*"
)
table(merip_final_data$strand, useNA = "ifany")
merip_gr <- GRanges(
  seqnames = chroms_atlas_merip,
  ranges = IRanges(start = merip_final_data$start, end = merip_final_data$end),
  strand = merip_final_data$strand,
  gene_name = merip_final_data$geneid
)

mut$in_m6A <- countOverlaps(mut_gr, atlas_gr) > 0
mut$in_merip <- countOverlaps(mut_gr, merip_gr) > 0
#mut$in_m6A <- countOverlaps(mut_gr, atlas_gr) > 0 | countOverlaps(mut_gr, mirep_gr) > 0
mut_m6A <- filter(mut, in_m6A)
mut_merip <- filter(mut, in_merip)
...
mut2 <- mut2 %>%
  left_join(
    histologies%>% dplyr::select(Tumor_Sample_Barcode, 
...
nrow(mut2) - nrow(mut3)
# Save as CSVs
write.csv(mut2, "mut2_all_mutations_histologies_test.csv", row.names = FALSE)
write.csv(mut3, "mut3_unique_patient_gene_test.csv", row.names = FALSE)
...
write.csv(mut_summary, "mut_summary_all_gnomad4.csv", row.names = FALSE)
write.csv(mut_summary_recurrent, "mut_summary_recurrent_gnomad4.csv", row.names = FALSE)
...
print(gene_summary %>% dplyr::select(Hugo_Symbol, n_patients, n_tumor_types) %>% head(5))
# Top genes 
...
patient_burden <- patient_gene_matrix2 %>%
  mutate(total_mutations = rowSums(dplyr::select(., -Kids.First.Participant.ID, -harmonized_diagnosis)))
...
patient_burden_group <- patient_gene_matrix %>%
  mutate(total_mutations = rowSums(dplyr::select(., -Kids.First.Participant.ID, -cancer_group, -harmonized_diagnosis)))
...
# HIGH gnomAD AF VARIANTS (>0.005)
# get the variants that would normally be filtered out
high_af_variants <- silent_variants_df %>%
  filter(Tumor_Sample_Barcode %in% histologies$Kids_First_Biospecimen_ID) %>%
  filter(str_detect(HGVSc, "^c\\.[0-9]+A>[ACGT]")) %>%
  mutate(across(all_of(num_cols), ~ as.numeric(.))) %>%
  mutate(
    gnomAD_4_AF = as.numeric(gnomAD_4_AF),
    gnomAD_4_AF = ifelse(is.na(gnomAD_4_AF), 0, gnomAD_4_AF)
  ) %>%
  filter(gnomAD_4_AF >= 0.005) %>%  # Keep high AF 
  filter(!is.na(Start_Position) & !is.na(End_Position)) %>%
  mutate(
    fracN = as.numeric(n_alt_count) / as.numeric(n_depth),
    fracT = as.numeric(t_alt_count) / as.numeric(t_depth)
  ) %>%
  filter(!is.na(fracN) & !is.na(fracT))  # Remove missing VAF 

cat("Total variants with gnomAD 4 AF ≥ 0.005:", nrow(high_af_variants), "\n")

# Summary stats
cat("\ngnomAD AF distribution:\n")
print(summary(high_af_variants$gnomAD_4_AF))

cat("\nTumor VAF distribution:\n")
print(summary(high_af_variants$fracT))

cat("\nNormal VAF distribution:\n")
print(summary(high_af_variants$fracN))

# Classify variants likely somatic vs germline
high_af_variants <- high_af_variants %>%
  mutate(
    vaf_diff = fracT - fracN,
    likely_somatic = case_when(
      fracN < 0.1 & fracT > 0.1 ~ "Likely Somatic (tumor-only)",
      abs(fracT - fracN) < 0.1 & fracN > 0.3 ~ "Likely Germline (matched)",
      fracT > fracN + 0.2 ~ "Possible Somatic (enriched in tumor)",
      TRUE ~ "Ambiguous"
    )
  )

cat("\nClassification of high AF variants:\n")
print(table(high_af_variants$likely_somatic))

# Plot 1 Tumor vs Normal VAF scatter plot
p_high_af_scatter <- ggplot(high_af_variants, aes(x = fracN, y = fracT, color = gnomAD_4_AF)) +
  geom_point(alpha = 0.6, size = 2) +
  geom_abline(slope = 1, linetype = "dashed", color = "black") +
  geom_hline(yintercept = 0.1, linetype = "dotted", color = "red") +
  geom_vline(xintercept = 0.1, linetype = "dotted", color = "red") +
  scale_color_viridis_c(option = "plasma", name = "gnomAD 4\nAF") +
  labs(
    title = "Tumor vs Normal VAF for High gnomAD AF Variants (≥0.5)",
    subtitle = "Dashed line = equality; dotted lines = 0.1 VAF threshold",
    x = "Normal VAF (fracN)",
    y = "Tumor VAF (fracT)"
  ) +
  theme_minimal() +
...
print(p_high_af_scatter)
ggsave("high_gnomad_af_tumor_vs_normal_vaf.pdf", p_high_af_scatter, width = 10, height = 8)

# Plot Faceted by classification
p_high_af_facet <- ggplot(high_af_variants, aes(x = fracN, y = fracT)) +
  geom_point(aes(color = gnomAD_4_AF), alpha = 0.6, size = 2) +
  geom_abline(slope = 1, linetype = "dashed") +
  facet_wrap(~likely_somatic, scales = "free") +
  scale_color_viridis_c(option = "plasma", name = "gnomAD 4\nAF") +
  labs(
    title = "VAF Comparison by Variant Classification",
    x = "Normal VAF",
    y = "Tumor VAF"
  ) +
  theme_minimal()

print(p_high_af_facet)
ggsave("high_gnomad_af_by_classification.pdf", p_high_af_facet, width = 12, height = 8)

# Plot 3 VAF difference dist
p_vaf_diff <- ggplot(high_af_variants, aes(x = vaf_diff, fill = likely_somatic)) +
  geom_histogram(bins = 50, alpha = 0.7, position = "identity") +
  geom_vline(xintercept = 0, linetype = "dashed") +
  labs(
    title = "Distribution of VAF Difference (Tumor - Normal)",
    subtitle = "Positive values = enriched in tumor; Negative = enriched in normal",
    x = "VAF Difference (fracT - fracN)",
    y = "Count",
    fill = "Classification"
  ) +
  theme_minimal() +
...
print(p_vaf_diff)
ggsave("high_gnomad_af_vaf_difference.pdf", p_vaf_diff, width = 10, height = 6)

# Plot 4 Density plot showing all three 
library(tidyr)
high_af_long <- high_af_variants %>%
  dplyr::select(Hugo_Symbol, Chromosome, Start_Position, gnomAD_4_AF, fracT, fracN) %>%
  pivot_longer(cols = c(fracT, fracN), names_to = "Sample_Type", values_to = "VAF") %>%
  mutate(Sample_Type = ifelse(Sample_Type == "fracT", "Tumor", "Normal"))

p_density <- ggplot(high_af_long, aes(x = VAF, fill = Sample_Type)) +
  geom_density(alpha = 0.5) +
  geom_vline(xintercept = 0.5, linetype = "dashed", color = "gray30") +
  scale_fill_manual(values = c("Tumor" = "red", "Normal" = "blue")) +
  labs(
    title = "VAF Distribution: Tumor vs Normal (High gnomAD AF Variants)",
    subtitle = "Dashed line at VAF = 0.5 (expected for heterozygous germline)",
...
print(p_density)
ggsave("high_gnomad_af_vaf_density.pdf", p_density, width = 10, height = 6)

# likely somatic variants with high gnomAD AF
likely_somatic_high_af <- high_af_variants %>%
  filter(likely_somatic %in% c("Likely Somatic (tumor-only)", "Possible Somatic (enriched in tumor)")) %>%
  dplyr::select(Hugo_Symbol, Chromosome, Start_Position, Reference_Allele, Tumor_Seq_Allele2, 
         gnomAD_4_AF, fracT, fracN, vaf_diff, likely_somatic) %>%
  arrange(desc(vaf_diff))

cat("\n=== Variants with high gnomAD AF but possible somatic origin ===\n")
cat("Count:", nrow(likely_somatic_high_af), "\n\n")
print(head(likely_somatic_high_af, 20))
