# AUTO FILTER COMBINATION 
# applies all combinations of three filters and generates plots/tables

library(tidyverse)
library(ggplot2)

#mut2 <- read_csv("mut2_all_mutations_histologies_test.csv")

# output directory
dir.create("filter_combinations_output", showWarnings = FALSE)
...
# all filter combinations (2^3 = 8 combinations)
filter_combinations <- list(
  ...
  list(name = "All_Three", filters = c("gnomad", "vaf", "m6a"), label = "gnomAD + VAF + m6A")
)
...
for (combo in filter_combinations) {
  ...
  write_csv(gene_summary, 
            paste0("filter_combinations_output/gene_summary_", combo$name, ".csv"))
  ...
  if (nrow(gene_summary) > 0) {
    ...
    # Save plot
    ggsave(
      filename = paste0("filter_combinations_output/top_genes_", combo$name, ".png"),
      plot = plot,
      width = 10,
      height = 8,
      dpi = 300
    ...
    # save as PDF
    ggsave(
      filename = paste0("filter_combinations_output/top_genes_", combo$name, ".pdf"),
      plot = plot,
      width = 10,
      height = 8
    )
...
# comparison summary across all filter combinations
comparison_df <- map_df(names(all_results), function(combo_name) {
  tibble(
    Filter_Combination = combo_name,
    Filter_Label = filter_combinations[[which(map_chr(filter_combinations, "name") == combo_name)]]$label,
    N_Mutations = all_results[[combo_name]]$n_mutations,
    N_Genes = all_results[[combo_name]]$n_genes,
    N_Patients = all_results[[combo_name]]$n_patients,
    N_Tumor_Types = all_results[[combo_name]]$n_tumor_types
  )
})

write_csv(comparison_df, "filter_combinations_output/filter_comparison_summary.csv")
...
comparison_plot <- comparison_df %>%
  ...
  theme(legend.position = "bottom",
        axis.text.y = element_text(size = 9))
comparison_plot
ggsave("filter_combinations_output/filter_comparison_plot.png", 
       comparison_plot, width = 10, height = 6, dpi = 300)

cat("\n=== ANALYSIS COMPLETE ===\n")
cat("All outputs saved to: filter_combinations_output/\n")
cat("- Individual gene summaries (CSV)\n")
cat("- Top genes plots (PNG + PDF)\n")
cat("- Filter comparison summary (CSV)\n")
cat("- Filter comparison plot (PNG)\n")